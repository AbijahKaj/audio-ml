...
export interface MFCCConfig { sampleRate: number, melBands?: number, fftSize?: number, numCoeffs?: number }
...
export class MFCCAnalyzer {
  ...
  analyzeFrame(pcm: Float32Array): number[] {
    if (pcm.length !== this.fftSize) throw new Error('PCM frame must be fftSize');
   ...
   */
  private dct(vector: number[]): number[] {
