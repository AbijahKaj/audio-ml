...
export class TDTJointNetwork {
  ...
  forward(
    ...
    if (!this.w.durationLinear) {
      throw new Error('TDT model requires durationLinear in JointWeights');
    }
    const durationLogits = linearForward(
      b,
      joint,
      this.w.durationLinear.weight,
      this.w.durationLinear.bias,
    );
    b.dispose(joint);
